...
class Sender(object):
    def __init__(
        self, prename, lastname, street, zip_code, place, company="", country=""
    ):
    ...
    def validate(self):
        if self.recipient is None or not self.recipient.is_valid():
            raise PostcardCreatorException(
                "Not all required attributes in recipient set"
            )
        if self.recipient is None or not self.recipient.is_valid():
...
class PostcardCreator(object):
    ...
    def __getattr__(self, method_name):
        def method(*args, **kwargs):
            logger.debug(
                "Forwarding method to implementation {}: '{}'".format(
                    self.token.token_implementation, method_name
                )
            )
