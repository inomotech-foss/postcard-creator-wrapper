...
class PostcardCreatorLegacy(PostcardCreatorBase):
    ...
    def _do_op(self, method, endpoint, **kwargs):
        ...
        if response.status_code not in [200, 201, 204]:
            e = PostcardCreatorException(
                "error in request {} {}. status_code: {}".format(
    ...
    def send_free_card(self, postcard, mock_send=False, **kwargs):
        ...
        asset_response = self._upload_asset(
            user, card_id=card_id, picture_stream=picture_stream
        )
        ...
        self._set_svg_page(
            ...
            user_id,
            card_id,
            self._get_frontpage(asset_id=asset_response["asset_id"]),
        )
        self._set_svg_page(
            2,
            user_id,
    ...
    def _create_card(self, user):
        ...
        mailing_payload = {
            "name": "Mobile App Mailing {}".format(
    ...
    def _upload_asset(self, user, card_id, picture_stream):
        ...
        return {"asset_id": asset_id, "response": response}
    ...
    def _set_svg_page(self, page_number, user_id, card_id, svg_content):
        ...
        endpoint = "/users/{}/mailings/{}/pages/{}".format(
            user_id, card_id, page_number
        )
    ...
    def _get_backpage(self, sender, recipient, message):
        ...
        return (
            svg.replace("{first_name}", _encode_text(recipient.prename))
            ...
            .replace(
                "{sender_name}",
                _encode_text(sender.prename) + " " + _encode_text(sender.lastname),
            )
            ...
            .replace("{message}", _encode_text(message))
        )
